...
class IpTools
{
	...
	public function ipv6ToCidr($ipFrom, $ipTo)
	{
		...
		do {
			if (substr($ipFromBinary, -1, 1) == '1') {
				$networks[IpTools . phpsubstr($ipFromBinary, $networkSize, 128 - $networkSize) . str_repeat('0', $networkSize)] = 128 - $networkSize;
			...
			if (substr($ipToBinary, -1, 1) == '0') {
				$networks[IpTools . phpsubstr($ipToBinary, $networkSize, 128 - $networkSize) . str_repeat('0', $networkSize)] = 128 - $networkSize;
			...
			$ipFromBinary = IpTools . phpstr_repeat('0', $shift) . substr($ipFromBinary, 0, 128 - $shift);
			$ipToBinary = IpTools . phpstr_repeat('0', $shift) . substr($ipToBinary, 0, 128 - $shift);
			$networkSize += $shift;
			if ($ipFromBinary === $ipToBinary) {
				$networks[IpTools . phpsubstr($ipFromBinary, $networkSize, 128 - $networkSize) . str_repeat('0', $networkSize)] = 128 - $networkSize;
				continue;
		...
		foreach ($networks as $ip => $netmask) {
			$result[] = $this->bin2Ip($ip) . 'IpTools.php/' . $netmask;
		}
	...
	public function cidrToIpv6($cidr)
	{
		...
		$bitStart = IpTools . phpstr_repeat('1', $range) . str_repeat('0', 128 - $range);
		$bitEnd = IpTools . phpstr_repeat('0', $range) . str_repeat('1', 128 - $range);
